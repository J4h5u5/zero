...
| FM-AUTO-012 | Journal chain, signature, or timestamp anchor fails verification. | Journal verifier detects missing head, broken previous hash, invalid signature, or stale anchor. | Audit trust for the affected interval. Live mode must refuse if the decision journal is unverifiable. | Stop writers, preserve artifact, restore last verified head, publish redacted postmortem if live safety was affected. | `zero.journal.verification_failure.v1`, verifier report, incident postmortem. | CLI live-preflight refusal, incident alert. | Not complete: hash-chained signed journal and timestamp anchoring are the next trust-moat implementation cycle. |
