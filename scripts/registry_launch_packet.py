...
def build_packet() -> dict[str, Any]:
    ...
    channels = [
        ...
        },
        {
            ...
            "evidence": [
                ...
                "anonymous pull currently returns unauthorized",
    ...
    return {
        ...
        "summary": {
            "default_distribution": "GitHub Release, public Homebrew tap, PyPI zero-engine, crates.io zero-os, Docker Hub getzero/zero, legacy authenticated GHCR smoke evidence, and GHCR product-image workflow readiness",
            "package_registries_enabled": True,
            "current_release": "v0.1.2",
            "policy": "PyPI zero-engine is published through Trusted Publishing; crates.io zero-os is published manually with a least-privilege token until tokenless publishing is available; Docker Hub getzero/zero is published with anonymous pull evidence; GHCR ghcr.io/zero-intel/zero is published but remains public-pull pending until package visibility is fixed.",
        },
        "channels": channels,
        "checks": checks,
