...
- [ ] PyPI `zero-engine` release, if included, uses `python-release.yml` Trusted Publishing; crates.io `zero-os` publication, if included, records owner evidence and yank steps; Docker Hub and GHCR publication remains disabled unless this release explicitly includes an ownership-proof section.
